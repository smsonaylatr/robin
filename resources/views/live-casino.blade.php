...
@section('title', 'Canlı Casino Oyunları - ' . ($settings->site_adi ?? 'BetNow'))
...
<div class="min-h-screen">
    ...
    <section class="relative overflow-hidden max-w-[1400px] mx-auto px-4">
        <div class="swiper live-casino-slider">
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    ...
                    <div class="relative w-full overflow-hidden">
                        <img src="{{ asset($slider->gorsel) }}" alt="Slider" class="block w-full h-auto object-contain">
                    </div>
                    @if($slider->url)
                    </a>
                    @endif
        ...
        <div class="max-w-7xl mx-auto">
                ...
                <div class="relative overflow-hidden rounded-xl bg-gradient-to-br from-zinc-900/50 via-zinc-900/30 to-transparent border border-zinc-800/50 p-4">
                    ...
                    <div class="relative flex items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-xl font-bold bg-clip-text text-transparent" style="background: linear-gradient(to right, #ebff00, #ebff00, #ebff00); -webkit-background-clip: text; background-clip: text; color: transparent;">Canlı Casino Sağlayıcıları</h2>
                            </div>
                        </div>
                        ...
                        <div class="relative max-w-xs">
                            ...
                            <input type="text" id="provider-search" placeholder="Sağlayıcı ara..." 
                                   class="w-full pl-10 pr-4 py-2 bg-zinc-800/50 border border-zinc-700/50 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-all duration-200 text-sm">
            ...
            <div class="provider-scroll-container overflow-x-auto" id="providers-container">
                <div class="flex gap-3 min-w-max px-2 py-2">
                        ...
                        <div class="relative overflow-hidden rounded-lg border px-4 py-3 transition-all duration-300 transform hover:scale-105 hover:shadow-lg min-w-[120px]" style="background: linear-gradient(to bottom right, rgba(235, 255, 0, 0.2), rgba(235, 255, 0, 0.1), transparent); border-color: rgba(235, 255, 0, 0.3);" onmouseover="this.style.borderColor='rgba(235, 255, 0, 0.5)'; this.style.boxShadow='0 10px 25px rgba(235, 255, 0, 0.2)';" onmouseout="this.style.borderColor='rgba(235, 255, 0, 0.3)'; this.style.boxShadow='none';">
                            ...
                            <div class="relative flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-zinc-400 text-xs">{{ $providers->count() }} sağlayıcı</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="provider-card group cursor-pointer flex-shrink-0" data-provider="{{ $provider->code }}" data-name="{{ strtolower($provider->name) }}">
                        <div class="relative overflow-hidden rounded-lg bg-gradient-to-br from-zinc-800/50 via-zinc-800/30 to-transparent border border-zinc-700/50 px-3 py-3 hover:border-zinc-600/50 transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-zinc-800/50 min-w-[100px]">
                            ...
                            <div class="relative text-center">
                                ...
                                <div class="w-24 h-6 mx-auto bg-gradient-to-br from-zinc-700/50 to-zinc-800/50 rounded-lg flex items-center justify-center border border-zinc-600/30 overflow-hidden">
                                    <div class="w-full h-full bg-gradient-to-r from-zinc-600 to-zinc-700 rounded-sm flex items-center justify-center px-1">
                                        <span class="text-white text-xs font-bold whitespace-nowrap">{{ strtoupper($provider->name) }}</span>
                                    </div>
                                </div>
                                
                                <!-- Sadece oyun sayısı -->
                                <div class="mt-2 text-center">
                                    <p class="text-zinc-400 text-xs">{{ $provider->count }} oyun</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="relative">
                ...
                <input type="text" id="game-search" placeholder="Oyun ara..." 
                       class="w-full pl-10 pr-4 py-3 bg-zinc-800/50 border border-zinc-700/50 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-all duration-200">
                <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                    <button id="clear-search" class="text-zinc-400 hover:text-white transition-colors hidden">
    ...
    <section class="py-8 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            <div id="games-container" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2">
                ...
                <div class="game-card group relative overflow-hidden rounded-xl hover:bg-zinc-700/50 transition-all duration-300 transform hover:scale-105 min-h-[220px]" data-provider="{{ $game->vendorcode }}">
                    <div class="relative">
                        ...
                        <div class="image-skeleton w-full h-40 bg-gradient-to-br from-zinc-700 to-zinc-800 rounded-t-xl relative overflow-hidden animate-pulse transition-opacity duration-300">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center">
                                <div class="relative">
                                    <div class="w-8 h-8 border-2 border-yellow-500/30 border-t-yellow-500 rounded-full animate-spin"></div>
                                    <div class="absolute inset-0 w-8 h-8 border-2 border-yellow-500/10 border-t-yellow-500/50 rounded-full animate-spin" style="animation-delay: -0.5s;"></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <img src="{{ $game->cover }}?v={{ time() }}" alt="{{ $game->game_name }}" class="w-full h-40 object-cover opacity-0 transition-opacity duration-500" 
                        ...
                        <div class="absolute top-0 left-0 right-0 h-40 bg-gradient-to-b from-black/90 via-black/80 to-black/90 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center backdrop-blur-sm">
                            ...
                            @endphp
                            @if($fakeApiActive == 1 && isset($game->ikincilmi) && $game->ikincilmi == 1)
                                @php
                            ...
                            @else
                                <a href="/GameLaunch/{{ $game->game_code ?? $game->id }}" 
                                   class="relative overflow-hidden rounded-lg bg-gradient-to-br border py-2 px-4 hover:border-yellow-500/50 transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-yellow-500/20 group">
                                    <div class="absolute inset-0 bg-[url('{{ asset('assets/noise.png') }}')] opacity-30"></div>
                                    <div class="absolute inset-0 bg-gradient-to-r from-yellow-500/10 via-yellow-500/5 to-transparent"></div>
                                    <div class="relative flex items-center justify-center gap-2">
                    ...
                    <div class="p-3 bg-zinc-800/30 border-t border-zinc-700/50">
                        ...
                        <p class="text-zinc-400 text-xs mt-1">{{ $game->provider->name ?? 'Live Casino' }}</p>
                    </div>
                </div>
                @endforeach
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const swiper = new Swiper('.live-casino-slider', {
        loop: true,
        autoplay: {
            delay: 5000,
...
function loadGames() {
    ...
    const url = new URL('/api/live-casino-games', window.location.origin);
...
function createGameCard(game) {
    ...
    card.className = 'game-card group relative overflow-hidden rounded-xl hover:bg-zinc-700/50 transition-all duration-300 transform hover:scale-105';
    ...
    const fakeApiActive = {{ \App\Models\Ayarlar::getSettings()->fakeapi ?? 0 }};
    ...
    card.innerHTML = `
        <div class="relative">
            <!-- Loading Skeleton with Shimmer Effect -->
            <div class="image-skeleton w-full h-32 bg-gradient-to-br from-zinc-700 to-zinc-800 rounded-t-xl relative overflow-hidden animate-pulse">
                <div class="absolute inset-0 bg-[url('{{ asset('assets/noise.png') }}')] opacity-30"></div>
            ...
            <!-- Actual Image (hidden initially) -->
            <img src="${game.cover}?v=${Date.now()}" alt="${game.game_name}" class="w-full h-32 object-cover opacity-0 transition-opacity duration-1000" 
                 loading="eager" decoding="sync"
                 onload="setTimeout(() => { 
                     this.style.opacity='1'; 
            ...
            <!-- Hover overlay -->
            <div class="absolute top-0 left-0 right-0 h-32 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center">
                <h3 class="text-white font-bold text-sm text-center mb-3 px-2">${game.game_name}</h3>
                <a href="${gameUrl}" class="bg-yellow-500/80 hover:bg-yellow-500 text-white font-semibold py-2 px-4 rounded-lg transition-colors text-sm">
                    Oyna
                </a>
            </div>
        ...
        <div class="p-3 bg-zinc-800/50 border-t border-zinc-700/50">
            <h3 class="text-white font-semibold text-sm mb-1 truncate" title="${game.game_name}">${game.game_name}</h3>
            <p class="text-zinc-400 text-xs truncate" title="${game.provider_name || 'Bilinmeyen Sağlayıcı'}">${game.provider_name || 'Bilinmeyen Sağlayıcı'}</p>
        </div>
    `;
