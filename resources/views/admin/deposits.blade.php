...
@section('title', 'Para Yatırma Talepleri')
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="stat-card-value">{{ $deposits->total() }}</p>
                </div>
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="stat-card-value">{{ $deposits->where('durum', 0)->count() }}</p>
                </div>
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="stat-card-value">{{ $deposits->where('durum', 1)->count() }}</p>
                </div>
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="stat-card-value">{{ number_format($deposits->where('durum', 1)->sum('miktar'), 2) }} TL</p>
                </div>
    ...
    <div class="content-card">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="text-sm text-gray-400">
                {{ $deposits->firstItem() }}-{{ $deposits->lastItem() }} / {{ $deposits->total() }} talep
            </div>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-zinc-700">
                    ...
                    <tr class="hover:bg-zinc-800/50 transition-colors">
                        <td class="py-4 px-4 text-sm text-gray-300">#{{ $deposit->id }}</td>
                        <td class="py-4 px-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-medium text-white">{{ $deposit->user->name ?? 'Bilinmiyor' }}</div>
                                    <div class="text-sm text-gray-400">ID: {{ $deposit->uye }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 px-4">
                            <div class="text-sm">
                                <div class="text-white font-medium">{{ number_format($deposit->miktar, 2) }} TL</div>
                                @if($deposit->bonus)
                                    <div class="text-gray-400">Bonus: {{ $deposit->bonus }} TL</div>
                                @endif
                            </div>
                        </td>
                        <td class="py-4 px-4">
                        ...
                        <td class="py-4 px-4 text-sm text-gray-300">
                            {{ $deposit->tarih ? $deposit->tarih->format('d.m.Y H:i') : '-' }}
                        </td>
                        <td class="py-4 px-4">
                            @if($deposit->durum == 0)
                        ...
                        <td class="py-4 px-4">
                            <div class="flex items-center gap-2">
                                ...
                                @if($deposit->durum == 0)
                                    <form method="POST" action="{{ route('admin.deposits.approve', $deposit->id) }}" class="inline">
                                        @csrf
                                    ...
                                    <form method="POST" action="{{ route('admin.deposits.reject', $deposit->id) }}" class="inline">
                                        @csrf
        ...
        <div class="flex items-center justify-between mt-6 pt-6 border-t border-zinc-700">
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    <a href="{{ $deposits->previousPageUrl() }}" class="px-3 py-2 text-white bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors">Önceki</a>
                @endif
                ...
                @if($deposits->hasMorePages())
                    <a href="{{ $deposits->nextPageUrl() }}" class="px-3 py-2 text-white bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors">Sonraki</a>
                @else
