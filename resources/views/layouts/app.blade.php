...
<head>
    ...
    <title>@yield('title', ($settings->site_adi ?? 'BetNow') . ' - Türkiye\'nin En Güvenilir Online Casino Platformu')</title>
...
<body class="bg-black text-white">
    ...
    <div class="flex min-h-screen flex-col relative bg-black/60 overflow-x-hidden">
                ...
                <div class="flex items-center justify-between h-12 sm:h-14">
                    <a class="text-2xl font-bold inline-block relative z-30 w-fit" href="/" style="color: #ebff00;">
                        ...
                        <img alt="Betnow" loading="lazy" width="200" height="200" decoding="async" 
                             style="width: {{ $settings->logo_size ?? 120 }}px; height: auto;"
                             class="h-auto p-2" src="{{ asset($logoPath) }}"/>
                    </a>
                    <div class="flex items-center gap-1.5 sm:gap-4">
                                        ...
                                        <div class="px-3 pb-2">
                                            ...
                                            <div class="p-3 rounded-lg border border-zinc-800 bg-zinc-900/70">
                                                <div class="flex items-center gap-3">
                                                    <div class="relative">
                                                        <div class="relative w-10 h-10 rounded-md bg-gradient-to-br {{ ''.$currentVip['gradient'] }} flex items-center justify-center text-black font-extrabold text-[10px] ring-1 ring-white/10">
                                                    ...
                                                    <div class="flex-1">
                                                        <div class="flex items-center justify-between">
                                                            <div class="text-sm text-white font-semibold tracking-wide">{{ $currentVip['name'] }}</div>
                                                            <div class="text-[10px] text-zinc-300 bg-zinc-800/60 border border-zinc-700/60 rounded px-1.5 py-0.5">₺{{ number_format($last30Deposit, 0) }}</div>
                                                        </div>
                                                        <div class="mt-2 h-2 rounded-full border border-zinc-700/60 bg-zinc-800 overflow-hidden">
                                                            <div class="h-full bg-red-500/80" style="width: {{ $progressPercent }}%"></div>
                                                        </div>
                                                        <div class="mt-1.5 flex items-center justify-between text-[10px]">
                                                            @if($nextVip)
                                                                <span class="text-zinc-300">Sonraki: <span class="text-white font-medium">{{ $nextVip['name'] }}</span> (₺{{ number_format($nextMin,0) }})</span>
                                                                <span class="text-zinc-300">Kalan: <span class="text-white font-medium">₺{{ number_format($remaining,0) }}</span></span>
                                                            @else
                        ...
                        <div class="flex items-center space-x-4">
                            <a href="{{ route('login') }}" onclick="console.log('Giriş Yap butonuna tıklandı');" class="px-4 py-2 text-zinc-300 hover:text-white transition-colors">Giriş Yap</a>
                            <a href="{{ route('register') }}" onclick="console.log('Kayıt Ol butonuna tıklandı');" class="px-4 py-2 rounded-lg transition-colors" style="position: relative; z-index: 1000; background-color: #ebff00; color: #000;">Kayıt Ol</a>
                        </div>
                        @endauth
                    </div>
                </div>
            ...
            <div class="py-6 px-4 flex-1 h-full overflow-y-auto">
                ...
                <div class="mb-3 floating-menu">
                    <div class="space-y-3">
                        ...
                        <a class="sidebar-menu-item ripple-effect glow-effect group flex items-center gap-2 px-3 py-2 rounded-lg glass-enhanced border border-zinc-600/30" href="{{ route('live-casino') }}">
                ...
                <div class="mb-3 floating-menu">
                    <div class="space-y-3">
                        <a class="sidebar-menu-item ripple-effect group flex items-center gap-2 px-3 py-2 rounded-lg" href="{{ route('sports') }}">
                            <div class="sidebar-icon-container w-7 h-7 rounded-lg flex items-center justify-center bg-zinc-700/50">
                ...
                <div class="mb-3 floating-menu">
                    <div class="space-y-3">
                        <a class="sidebar-menu-item ripple-effect group flex items-center gap-2 px-3 py-2 rounded-lg" href="{{ route('bonus') }}">
                            <div class="sidebar-icon-container w-7 h-7 rounded-lg flex items-center justify-center bg-zinc-700/50">
                ...
                <!-- Canlı Destek ve Social Media Links -->
                    ...
                    <div class="flex justify-center space-x-3 px-2">
                        @if($settings && $settings->telegram)
                            <a href="{{ $settings->telegram }}" target="_blank" class="w-10 h-10 rounded-lg bg-zinc-800/70 hover:bg-zinc-700/70 border border-zinc-700/60 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-md" style="hover:shadow: rgba(235, 255, 0, 0.2);" title="Telegram">
                                <img src="{{ asset('images/telegram.png') }}" alt="Telegram" class="w-6 h-6 object-contain">
                            </a>
                        @endif
                        @if($settings && $settings->whatsapp)
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $settings->whatsapp) }}" target="_blank" class="w-10 h-10 rounded-lg bg-zinc-800/70 hover:bg-zinc-700/70 border border-zinc-700/60 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-md" style="hover:shadow: rgba(235, 255, 0, 0.2);" title="WhatsApp">
                                <img src="{{ asset('images/wp.png') }}" alt="WhatsApp" class="w-6 h-6 object-contain">
                            </a>
                        @endif
                        @if($settings && $settings->instagram)
                            <a href="{{ $settings->instagram }}" target="_blank" class="w-10 h-10 rounded-lg bg-zinc-800/70 hover:bg-zinc-700/70 border border-zinc-700/60 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-md" style="hover:shadow: rgba(235, 255, 0, 0.2);" title="Instagram">
                                <img src="{{ asset('images/insta.png') }}" alt="Instagram" class="w-6 h-6 object-contain">
                            </a>
                        @endif
                        @if($settings && $settings->twitter)
                            <a href="{{ $settings->twitter }}" target="_blank" class="w-10 h-10 rounded-lg bg-zinc-800/70 hover:bg-zinc-700/70 border border-zinc-700/60 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-md" style="hover:shadow: rgba(235, 255, 0, 0.2);" title="X (Twitter)">
                                <img src="{{ asset('images/x.png') }}" alt="X (Twitter)" class="w-6 h-6 object-contain">
                            </a>
                        @endif
                    </div>
                </div>
            ...
            <div class="py-6 px-4 flex-1 h-full overflow-y-auto">
                ...
                <div class="mb-3">
                    <div class="space-y-3">
                        ...
                        <a class="sidebar-menu-item ripple-effect group flex items-center gap-2 px-3 py-2 rounded-lg" href="{{ route('live-casino') }}">
                            <div class="sidebar-icon-container w-6 h-6 rounded-lg flex items-center justify-center bg-zinc-700/50">
                ...
                <!-- Canlı Destek ve Social Media Links -->
                    ...
                    <div class="flex justify-center space-x-3 px-2">
                        @if($settings && $settings->telegram)
                            <a href="{{ $settings->telegram }}" target="_blank" class="w-8 h-8 rounded-lg bg-zinc-800/70 hover:bg-zinc-700/70 border border-zinc-700/60 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-md" style="hover:shadow: rgba(235, 255, 0, 0.2);" title="Telegram">
                                <img src="{{ asset('images/telegram.png') }}" alt="Telegram" class="w-4 h-4 object-contain">
                            </a>
                        @endif
                        @if($settings && $settings->whatsapp)
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $settings->whatsapp) }}" target="_blank" class="w-8 h-8 rounded-lg bg-zinc-800/70 hover:bg-zinc-700/70 border border-zinc-700/60 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-md" style="hover:shadow: rgba(235, 255, 0, 0.2);" title="WhatsApp">
                                <img src="{{ asset('images/wp.png') }}" alt="WhatsApp" class="w-4 h-4 object-contain">
                            </a>
                        @endif
                        @if($settings && $settings->instagram)
                            <a href="{{ $settings->instagram }}" target="_blank" class="w-8 h-8 rounded-lg bg-zinc-800/70 hover:bg-zinc-700/70 border border-zinc-700/60 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-md" style="hover:shadow: rgba(235, 255, 0, 0.2);" title="Instagram">
                                <img src="{{ asset('images/insta.png') }}" alt="Instagram" class="w-4 h-4 object-contain">
                            </a>
                        @endif
                        @if($settings && $settings->twitter)
                            <a href="{{ $settings->twitter }}" target="_blank" class="w-8 h-8 rounded-lg bg-zinc-800/70 hover:bg-zinc-700/70 border border-zinc-700/60 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-md" style="hover:shadow: rgba(235, 255, 0, 0.2);" title="X (Twitter)">
                                <img src="{{ asset('images/x.png') }}" alt="X (Twitter)" class="w-4 h-4 object-contain">
                            </a>
                        @endif
                    </div>
                </div>
        ...
        <div class="flex-1 md:ml-64">
                            ...
                            <div class="relative p-6">
                                ...
                                <div class="space-y-3 mb-6">
                                    <div class="p-4 rounded-xl bg-zinc-800/50 border border-zinc-700/50">
                                        ...
                                        <div class="flex items-center justify-between">
                                            <span class="text-lg font-bold text-white" id="regUsername">{{ session('registered_username') }}</span>
                                            <button onclick="copyToClipboard('{{ session('registered_username') }}', this)" class="px-3 py-1 rounded-lg text-xs font-medium transition-all duration-200" style="background-color: rgba(235, 255, 0, 0.1); border: 1px solid rgba(235, 255, 0, 0.2); color: #ebff00;">
                                                Kopyala
                                            </button>
                                        </div>
                                    </div>
                                    <div class="p-4 rounded-xl bg-zinc-800/50 border border-zinc-700/50">
                                        ...
                                        <div class="flex items-center justify-between">
                                            <span class="text-lg font-bold text-white">{{ session('registered_password') }}</span>
                                            <button onclick="copyToClipboard('{{ session('registered_password') }}', this)" class="px-3 py-1 rounded-lg text-xs font-medium transition-all duration-200" style="background-color: rgba(235, 255, 0, 0.1); border: 1px solid rgba(235, 255, 0, 0.2); color: #ebff00;">
                                                Kopyala
                                            </button>
                                        </div>
                                    </div>
            ...
            @endphp
            @include('layouts.footer', ['payments' => $payments])
        </div>

        <!-- Mobile Bottom Navigation -->
        <div class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-sm border-t border-zinc-800">
            <div class="grid grid-cols-5 gap-0">
                <a href="{{ route('home') }}" class="flex flex-col items-center justify-center py-1 {{ request()->routeIs('home') ? 'text-red-500' : 'text-zinc-400' }} hover:text-white transition-colors">
                ...
                <a href="{{ route('casino') }}" class="flex flex-col items-center justify-center py-1 {{ request()->routeIs('casino') ? 'text-red-500' : 'text-zinc-400' }} hover:text-white transition-colors">
                ...
                <a href="{{ route('live-casino') }}" class="flex flex-col items-center justify-center py-1 {{ request()->routeIs('live-casino') ? 'text-red-500' : 'text-zinc-400' }} hover:text-white transition-colors">
                    <svg class="w-5 h-5 mb-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
...
<noscript><a href="https://www.livechat.com/chat-with/' . $licenseNumber . '/" rel="nofollow">Chat with us</a>, powered by <a href="https://www.livechat.com/?welcome" rel="noopener nofollow" target="_blank">LiveChat</a></noscript>';
            ...
            <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-zinc-900 via-zinc-900/95 to-zinc-800 border shadow-2xl" style="border-color: rgba(235, 255, 0, 0.3);">
                ...
                <div class="relative p-6 text-center">
                    ...
                    <div class="flex flex-col gap-3 pb-2">
                        <a href="{{ route('para-yatir') }}" class="w-full relative overflow-hidden rounded-lg py-3 px-6 transition-all duration-300 transform hover:scale-[1.02] group block" style="background: linear-gradient(to bottom right, rgba(235, 255, 0, 0.8), rgba(235, 255, 0, 0.6)); border: 1px solid rgba(235, 255, 0, 0.9);">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var userBalance = {{ Auth::guard('admin')->user()->bakiye ?? 0 }};
