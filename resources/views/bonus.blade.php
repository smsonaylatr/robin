    ...
    <section class="py-12 px-4">
        <div class="max-w-7xl mx-auto">
                ...
                <div class="group bg-zinc-900/50 backdrop-blur-sm rounded-2xl border border-zinc-800/50 hover:border-red-500/50 transition-all duration-500 hover:transform hover:scale-[1.02] hover:shadow-2xl hover:shadow-red-500/10 overflow-hidden">
                    ...
                    <div class="relative h-48 overflow-hidden bg-zinc-800/30">
                        @if($bonus->bonus_image)
                            <img src="{{ $bonus->bonus_image }}" alt="{{ $bonus->bonus_name }}" 
                    ...
                    <div class="p-6">
                                ...
                                @if($bonus->bonus_amount > 0)
                                    <div class="text-center">
                                        <p class="text-2xl font-bold text-green-400">
                                            {{ number_format($bonus->bonus_amount, 0) }} TL
                                        </p>
                        ...
                        <!-- Bonus Requirements -->
                        @if($bonus->cevrim > 0 || $bonus->maxtutar > 0 || $bonus->altlimit > 0)
                            ...
                            <div class="space-y-1 text-xs text-zinc-400">
                                @if($bonus->cevrim > 0)
                                    <div class="flex justify-between">
                                        ...
                                        <span class="text-white font-medium">{{ $bonus->cevrim }}x</span>
                                    </div>
                                @endif
                                @if($bonus->maxtutar > 0)
                                    <div class="flex justify-between">
                                        <span>Maksimum Tutar:</span>
                                        <span class="text-white font-medium">{{ number_format($bonus->maxtutar, 0) }} TL</span>
                                    </div>
                                @endif
                                @if($bonus->altlimit > 0)
                                    <div class="flex justify-between">
                                        <span>Minimum Yatırım:</span>
                                        <span class="text-white font-medium">{{ number_format($bonus->altlimit, 0) }} TL</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-3">
                            <button onclick="showBonusDetails({{ $bonus->id }})" class="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white px-3 py-1.5 rounded-lg transition-colors border border-zinc-700 hover:border-zinc-600 flex items-center justify-center gap-2 text-sm" title="Bonus Detayları">
                            ...
                            @if($bonus->aktif == '1')
                                @auth('admin')
                                    ...
                                    @elseif($canClaim)
                                        <button onclick="claimBonus({{ $bonus->id }}, event)" class="flex-1 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-3 py-1.5 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 text-sm flex items-center justify-center gap-2">
                                            <i class="fas fa-gift"></i> Bonusu Al
                                        </button>
                                    @else
                                        <button onclick="showDisabledReason('{{ $disabledReason }}')" class="flex-1 bg-zinc-700 hover:bg-zinc-600 text-zinc-300 px-3 py-1.5 rounded-lg font-semibold transition-colors text-sm flex items-center justify-center gap-2">
            ...
            <div class="text-center py-16">
                ...
                <a href="{{ route('home') }}" class="inline-flex items-center px-6 py-3 bg-red-500 hover:bg-red-600 text-white rounded-lg font-semibold transition-colors">
                    Ana Sayfaya Dön
                </a>
            </div>
            @endif
...
const bonusData = {
    @foreach($bonuses as $bonus)
    {{ $bonus->id }}: {
        name: @json($bonus->bonus_name),
        image: @json($bonus->bonus_image),
        description: @json($bonus->bonus_description),
        amount: {{ $bonus->bonus_amount }},
        percentage: {{ $bonus->yuzde }},
        maxAmount: {{ $bonus->maxtutar }},
        minDeposit: {{ $bonus->altlimit }},
        wagering: {{ $bonus->cevrim }},
        isDemo: {{ $bonus->deneme }},
        isWelcome: {{ $bonus->hosgeldin }},
        isDeposit: {{ $bonus->yatirim }},
        isLoss: {{ $bonus->kayip }},
        isActive: {{ $bonus->aktif }}
    },
    @endforeach
};
