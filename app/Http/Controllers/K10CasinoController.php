...
class K10CasinoController extends Controller
{
    ...
    /**
     * K10 Casino API'sine istek gönder
     */
    private function makeApiRequest($method, $params = [])
    {
        ...
        try {
            ...
            Log::info('K10 Casino API Request', [
                'method' => $method,
                'params' => $params,
            ...
            Log::error('K10 Casino API Error', [
                'method' => $method,
                'error' => $e->getMessage()
            ]);
            return null;
    ...
    public function directGameLaunch($vendorCode, $gameCode = null)
    {
        ...
        if (!$createUserResponse || $createUserResponse['status'] !== 0) {
            Log::warning('K10 Casino user creation failed', [
                'userCode' => $userCode,
        ...
        if ($launchResponse && $launchResponse['status'] === 0) {
            ...
            Log::error('K10 Casino game launch failed', [
                'userCode' => $userCode,
                'vendorCode' => $vendorCode,
                'gameCode' => $gameCode,
    ...
    {
        Log::info('K10 Casino Webhook received', [
    ...
    private function handleChangeBalance($data)
    {
        ...
        $description = "K10 Casino - {$vendorCode} - {$gameCode}";
