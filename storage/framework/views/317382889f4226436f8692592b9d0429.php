    ...
    <?php $__env->startSection('title', 'Para Yatır - ' . ($settings->site_adi ?? 'BetNow')); ?>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <?php if(session('error_message')): ?>
            <div class="bg-red-900/20 border border-red-500/30 rounded-lg p-4 mb-6">
                <p class="text-red-400 text-center font-medium"><?php echo e(session('error_message')); ?></p>
            </div>
        <?php endif; ?>
        ...
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <?php $__currentLoopData = $providers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $provider => $methods): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                    $providerNames = [
                ...
                <div class="mb-8">
                    <h2 class="text-xl font-bold text-white mb-4 text-center">
                        <i class="<?php echo e($providerIcons[$provider] ?? 'fas fa-credit-card'); ?> mr-2"></i>
                        <?php echo e($providerNames[$provider] ?? ucfirst($provider) . ' Ödeme Yöntemleri'); ?>
                    ...
                    <div class="space-y-4">
                        ...
                        <?php $__currentLoopData = $methods; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $method): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-lg overflow-hidden">
                                <div class="p-4 cursor-pointer hover:bg-zinc-800/30 transition-colors" onclick="togglePanel('<?php echo e($method->method_key); ?>-content')">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3">
                                            <?php if($method->image): ?>
                                                <img src="<?php echo e(asset($method->image)); ?>" 
                                                    alt="<?php echo e($method->method_name); ?>" class="w-auto h-12 rounded-lg" onerror="this.style.display='none'">
                                            <?php else: ?>
                                            ...
                                            <?php endif; ?>
                                            <div>
                                                <h3 class="text-white font-semibold"><?php echo e($method->method_name); ?> ile Para Yatır</h3>
                                                <div class="flex space-x-2 mt-1">
                                                    <span class="px-2 py-1 bg-green-500/20 text-green-400 text-xs rounded">Min: <?php echo e(number_format($method->min_amount)); ?>₺</span>
                                                    <span class="px-2 py-1 bg-blue-500/20 text-blue-400 text-xs rounded">Max: <?php echo e(number_format($method->max_amount)); ?>₺</span>
                                                </div>
                                            </div>
                                        </div>
                                        <svg class="w-5 h-5 text-zinc-400 transform transition-transform" id="<?php echo e($method->method_key); ?>-icon">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                        </svg>
                                    </div>
                                </div>
                                <div class="hidden border-t border-zinc-800" id="<?php echo e($method->method_key); ?>-content">
                                    <div class="p-6">
                                        <?php if($method->provider === 'oley' && $method->method_key === 'KOLAYHAVALE'): ?>
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <div class="grid grid-cols-3 gap-2 mb-3">
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="100" data-target="<?php echo e($method->method_key); ?>">100₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="500" data-target="<?php echo e($method->method_key); ?>">500₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="1000" data-target="<?php echo e($method->method_key); ?>">1000₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="2500" data-target="<?php echo e($method->method_key); ?>">2500₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="5000" data-target="<?php echo e($method->method_key); ?>">5000₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="10000" data-target="<?php echo e($method->method_key); ?>">10000₺</button>
                                                </div>
                                                <input type="number" id="<?php echo e($method->method_key); ?>-amount" name="amount" 
                                                    class="w-full bg-zinc-800 border border-zinc-700 text-white rounded-lg px-3 py-2"
                                                    placeholder="Özel miktar giriniz" min="<?php echo e($method->min_amount); ?>" max="<?php echo e($method->max_amount); ?>">
                                            </div>
                                        ...
                                        <?php elseif($method->provider === 'oley' && in_array($method->method_key, ['OLEY_CREDIT_CARD', 'OLEY_PAPARA', 'OLEY_MEFETE', 'PARAZULA', 'POPY', 'PAYCO'])): ?>
                                            <!-- OleyPayment diğer yöntemleri için miktar seçimi -->
                                            <div class="mb-4">
                                                <label class="block text-sm font-medium text-zinc-300 mb-2">Yatırılacak Miktar</label>
                                                <div class="grid grid-cols-3 gap-2 mb-3">
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="<?php echo e($method->min_amount); ?>" data-target="<?php echo e($method->method_key); ?>"><?php echo e(number_format($method->min_amount)); ?>₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="100" data-target="<?php echo e($method->method_key); ?>">100₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="500" data-target="<?php echo e($method->method_key); ?>">500₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="1000" data-target="<?php echo e($method->method_key); ?>">1000₺</button>
                                                    <button type="button" class="amount-btn bg-zinc-800 hover:bg-zinc-700 text-white py-2 px-4 rounded transition-colors" data-amount="2500" data-target="<?php echo e($method->method_key); ?>">2500₺</button>
                                        ...
                                        <?php elseif($method->provider === 'extra' && $method->method_key === 'EXTRA_KRIPTO'): ?>
                                            <!-- Extra Cüzdan Kripto için network seçimi -->
                                            <div class="mb-4">
                                                <label class="block text-sm font-medium text-zinc-300 mb-2">Ağ (Network)</label>
                                                <select id="<?php echo e($method->method_key); ?>-network" class="w-full bg-zinc-800 border border-zinc-700 text-white rounded-lg px-3 py-2">
                                        ...
                                        <div class="text-right">
                                            <?php if($method->provider === 'oley'): ?>
                                                <button class="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-lg transition-colors" 
                                                        onclick="submitOleyPayment('<?php echo e($method->method_key); ?>')">
                                                    <i class="fa fa-credit-card mr-2"></i>
                                                    PARA YATIR
                                                </button>
                                            <?php else: ?>
                                                <button class="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-lg transition-colors" 
                                                        onclick="submitPaymentForm('<?php echo e($method->method_key); ?>')">
                                                    <i class="fa fa-credit-card mr-2"></i>
                                                    PARA YATIR
                                                </button>
                                            <?php endif; ?>
    ...
    var userBalance = <?php echo e((float)$user->bakiye); ?>;
    var bonusActive = <?php echo e((int)($user->yasakbonus ?? 0)); ?>;
    ...
    fetch('<?php echo e(route("payment.method")); ?>', {
        method: 'POST',
        body: formData,
        headers: {
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/vhosts/robinbet118.com/httpdocs/resources/views/para-yatir.blade.php ENDPATH**/ ?>