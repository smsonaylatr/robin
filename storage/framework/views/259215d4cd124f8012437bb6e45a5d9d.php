<?php $__env->startSection('title', 'Hesabım - ' . ($settings->site_adi ?? 'BetNow')); ?>
...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                <div class="w-16 h-16 bg-red-500/20 rounded-full flex items-center justify-center">
                    <span class="text-2xl font-bold text-red-400"><?php echo e(substr($user->username, 0, 1)); ?></span>
                </div>
                <div>
                    <h2 class="text-xl font-bold text-white"><?php echo e($user->username); ?></h2>
                    <p class="text-zinc-400">Üye ID: <?php echo e($user->id); ?></p>
                    <p class="text-zinc-400">Kayıt: <?php echo e(\Carbon\Carbon::parse($user->created_at)->format('d.m.Y')); ?></p>
                </div>
            </div>
            <div class="text-right">
                <p class="text-zinc-400 text-sm">Bakiye</p>
                <p class="text-3xl font-bold text-white"><?php echo e($user->parabirimi); ?><?php echo e(number_format($user->bakiye, 2)); ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <a href="<?php echo e(route('para-yatir')); ?>" class="bg-black/30 border border-zinc-700/60 rounded-lg p-4 hover:border-red-500/50 hover:bg-black/40 transition-colors">
        ...
        <a href="<?php echo e(route('para-cek')); ?>" class="bg-black/30 border border-zinc-700/60 rounded-lg p-4 hover:border-red-500/50 hover:bg-black/40 transition-colors">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 bg-red-500/10 rounded-full flex items-center justify-center">
                    <svg class="w-6 h-6 text-red-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <rect x="3" y="7" width="18" height="10" rx="2" fill="#111" stroke="currentColor" stroke-width="1.5"/>
        ...
        <a href="<?php echo e(route('hesap-hareketleri')); ?>" class="bg-black/30 border border-zinc-700/60 rounded-lg p-4 hover:border-red-500/50 hover:bg-black/40 transition-colors">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 bg-red-500/10 rounded-full flex items-center justify-center">
                    <svg class="w-6 h-6 text-red-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <a href="<?php echo e(route('aktif-bonuslarim')); ?>" class="bg-black/30 border border-zinc-700/60 rounded-lg p-4 hover:border-red-500/50 hover:bg-black/40 transition-colors">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 bg-red-500/10 rounded-full flex items-center justify-center">
                    <svg class="w-6 h-6 text-red-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-lg p-6 mb-8">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="<?php echo e(route('hesap-hareketleri')); ?>" class="text-red-400 hover:text-red-300 transition-colors">Tümünü Gör</a>
        </div>
        ...
        <?php if($recentTransactions->count() > 0): ?>
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 bg-zinc-800/30 rounded-lg">
                    <div class="flex items-center space-x-3">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center <?php echo e($transaction->type === 'win' ? 'bg-green-500/20' : ($transaction->type === 'bet' ? 'bg-red-500/20' : 'bg-blue-500/20')); ?>">
                            <span class="text-sm font-medium <?php echo e($transaction->type === 'win' ? 'text-green-400' : ($transaction->type === 'bet' ? 'text-red-400' : 'text-blue-400')); ?>">
                                <?php echo e($transaction->type === 'win' ? 'W' : ($transaction->type === 'bet' ? 'B' : 'T')); ?>

                            </span>
                        </div>
                        <div>
                            <p class="text-white text-sm"><?php echo e(ucfirst($transaction->type)); ?></p>
                            <p class="text-zinc-400 text-xs"><?php echo e(\Carbon\Carbon::parse($transaction->created_at)->format('d.m.Y H:i')); ?></p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-sm font-medium <?php echo e($transaction->type === 'win' ? 'text-green-400' : ($transaction->type === 'bet' ? 'text-red-400' : 'text-blue-400')); ?>">
        ...
        <div class="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-lg p-6">
            ...
            <?php if($deposits->count() > 0): ?>
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-zinc-800/30 rounded-lg">
                        <div>
                            <p class="text-white text-sm"><?php echo e($user->parabirimi); ?><?php echo e(number_format($deposit->miktar, 2)); ?></p>
                            <p class="text-zinc-400 text-xs"><?php echo e(ucfirst($deposit->tur)); ?> - <?php echo e(\Carbon\Carbon::parse($deposit->tarih)->format('d.m.Y H:i')); ?></p>
                        </div>
                        <span class="px-2 py-1 text-xs rounded-full <?php echo e($deposit->durum == 1 ? 'bg-green-500/20 text-green-400' : ($deposit->durum == 0 ? 'bg-yellow-500/20 text-yellow-400' : 'bg-red-500/20 text-red-400')); ?>">
                            <?php echo e($deposit->durum == 1 ? 'Onaylandı' : ($deposit->durum == 0 ? 'Beklemede' : 'Reddedildi')); ?>
        ...
        <div class="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-lg p-6">
            ...
            <?php if($withdrawals->count() > 0): ?>
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-zinc-800/30 rounded-lg">
                        <div>
                            <p class="text-white text-sm"><?php echo e($user->parabirimi); ?><?php echo e(number_format($withdrawal->miktar, 2)); ?></p>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/vhosts/robinbet118.com/httpdocs/resources/views/hesabim.blade.php ENDPATH**/ ?>